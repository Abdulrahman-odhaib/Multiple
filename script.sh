...
echo -e "${WHITE}This script will do 7_STEPS:
	
        1/ Gathering subdomains
        2/ Put all subdomains together
        3/ Resolving all subdomains to IP
        4/ Organization
	5/ See live domains
	6/ Open live domains in Chromium
	7/ Take screenshots 
"
...
if ping -c 1 "ayakalammayhm.$domain" &> /dev/null; then
    echo -e ">>>>>>>>>>>>>>>> ${RED}BAD: DNS Wildcard exists. Mission canceled."
    while true; do echo -e "\a"; done
    exit 1
else
    echo -e ">>>>>>>>>>>>>>>> ${BLUE}GOOD: DNS Wildcard doesn't exist. Continuing with the mission."
    sleep $delay
    ...
    echo "RECOGNIZE"
    cd /$USER/$domain/ ; mv live_host lastlive_host http_site http_site online/
    echo "************ THE TARGET FILE IS IN HOME DIRECTORY **********"
fi
